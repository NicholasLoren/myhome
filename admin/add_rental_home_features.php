...
<!-- Content Wrapper. Contains page content -->
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-12">
            <h1 class="m-0">HOMES FOR RENT FEATURES SECTION
              <li class="btn btn-sm btn-primary"style="float: right;">
                    <a href="<?php _link(); ?>listHome_rentals" aria-hidden="false"  title="View homes for rent" data-toggle="tooltip" data-placement="right" style="color: white;"><i class="fa fa-fast-backward"></i>&nbsp;Go to homes for rent</a>
                    </li> </h1>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
          ...
          <div class="col-md-12">
            ...
            <div class="card card-primary">
              <div class="card-header">
                <?php
                    $get_home = dbQuery("SELECT * FROM tbl_home_rentals WHERE home_code ='$home_code' LIMIT 1");
                      $count1 = dbNumRows($get_home);
                    if($count1 > 0) {
                      while($row1=dbFetchAssoc($get_home)){ 
                      $name = $row1['home_name']; ?>
                <h3 class="card-title">Add home Features for : <?php echo $name; ?> </h3>
              <?php } } ?>
              </div>
              <!-- /.card-header -->
              <!-- form start -->
               <form action="features/RHF_update.php" method="post">
<input type="hidden" id="home_code" name="home_code" value="<?php echo $home_code;?>" required>
                <div class="card-body">
                  <div class="row col-md-12 alert alert-success">
                    <?php
                    $get_features = dbQuery("SELECT * FROM tbl_rental_home_features WHERE home_code ='$home_code' LIMIT 1");
     ...
     <div class='alert alert-danger'>
        <button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
               This home has no extra features, add features by selecting from the list below.
      </div>";
                 } ?>
              </div>
                <!-- /.card-body -->
