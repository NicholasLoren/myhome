...
if (isset($_POST['submit'])) {
  ...
  //creating death code
  $result = @ dbQuery("SHOW TABLE STATUS LIKE  'tbl_office_rentals'");
  $data = @dbFetchAssoc($result);
  $next = $data['Auto_increment'];
  $prefix = "MYHOME/OFR/";
  $office_code = sprintf("%s%03s",$prefix,$next);
                    
  $existTime = dbNumRows(dbQuery("SELECT * FROM `tbl_office_rentals` WHERE `office_name`='$office_name'"));
  if($existTime>0){
    warningMsg("Rental office Already registered");
    }else{
        
  //insert  into database
  $office_rental = dbQuery("INSERT INTO tbl_office_rentals(`office_code`, `office_name`, `total_rooms`, `location`, `longitude`, `latitude`, `price`, `period`, `measurement`,  `description`, `office_type`, `accomodation_type`, `year_built`, `waiting_room`, `cooling`, `heating`, `parking`, `outdoor`, `listed_by`) VALUES('$office_code', '$office_name', '$total_rooms','$location', '$longitude', '$latitude', '$price', '$period', '$measurement', '$description', '$office_type',  '$accomodation_type', '$year_built', '$waiting_room','$cooling', '$heating', '$parking', '$outdoor','$listed_by')"); 

      if($office_rental){
        //insert office code into table office features
     dbQuery("INSERT INTO tbl_rental_office_features(`office_code`)VALUES('$office_code')");
         $msg = "
     <div class='alert alert-success alert-dismissible'>
        <button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
                New Rental Office Successfully Registered
      </div>";
      goToPage("listOffice_rentals");
    }
  } 
}
?>
<!-- Content Wrapper. Contains page content -->
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0">RENTAL OFFICES SECTION</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
           <li class="<?php echo ($role==1 or $role==3)?'':' hide';?> btn btn-sm btn-primary" style="float: right; margin-right: 5px;">
            <a href="<?php _link(); ?>listOffice_rentals"  aria-hidden="false" title="Back to Rental Offices" data-toggle="tooltip" data-placement="right" style="color: white;"><i class="fa fa-fast-backward"></i> &nbsp;Back </a>&nbsp;&nbsp; 
            </li>   
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
                ...
                <div class="card-body">
                  <div class="row">
                    <div class="form-group col-md-6">
                      ...
                      <input type="text" name="name" class="form-control"  placeholder="Enter Home Name">
                    </div>
                    <div class="form-group col-md-6">
                    ...
                    <div class="form-group col-md-2">
                      ...
                      <input type="text" name="total_rooms" class="form-control"  placeholder="Enter Longitude">
                    </div>
                    <div class="form-group col-md-2">
                      <label for="exampleInputtext1">Waiting Rooms</label>
                      <input type="text" name="waiting_room" class="form-control"  placeholder="Enter Longitude">
                    </div>
                    <div class="form-group col-md-2">
                    ...
                    <div class="form-group col-md-2">
                      ...
                      <select name="office_type" class="form-control select2" style="width: 100%;">
                        <option selected="selected">Choose office type</option>
                       <option value="Company Office">Company Office</option>
                        <option value="Branch Office">Branch Office</option>
                        <option value="Bank/Sacco office">Bank/Sacco office</option>
                      </select>
                    </div>
                    <div class="form-group col-md-2">
