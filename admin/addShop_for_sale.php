...
if (isset($_POST['submit'])) {
  ...
  //creating death code
  $result = @ dbQuery("SHOW TABLE STATUS LIKE  'tbl_shop_for_sale'");
  $data = @dbFetchAssoc($result);
  $next = $data['Auto_increment'];
  $prefix = "MYHOME/SS/";
  $shop_code = sprintf("%s%03s",$prefix,$next);
                    
  $existTime = dbNumRows(dbQuery("SELECT * FROM `tbl_shop_for_sale` WHERE `shop_name`='$shop_name'"));
  if($existTime>0){
    warningMsg("Shop for sale Already registered");
    }else{
        
  //insert  into database
  $shop_sale = dbQuery("INSERT INTO tbl_shop_for_sale(`shop_code`, `shop_name`, `location`, `longitude`, `latitude`, `price`, `measurement`,  `description`, `shop_type`, `accomodation_type`, `year_built`, `total_rooms`, `store_room`, `cooling`, `heating`, `parking`, `outdoor`, `listed_by`) VALUES('$shop_code', '$shop_name', '$location', '$longitude', '$latitude', '$price', '$measurement', '$description', '$shop_type',  '$accomodation_type', '$year_built','$total_rooms', '$store_room','$cooling', '$heating', '$parking', '$outdoor','$listed_by')");
   //insert shop code into table shop features
     dbQuery("INSERT INTO tbl_shop_forsale_features(`shop_code`)VALUES('$shop_code')"); 

      if($shop_sale){
         $msg = "
     <div class='alert alert-success alert-dismissible'>
        <button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
                New Shop for sale Successfully Registered
      </div>";
      goToPage("shops_for_sale");
    }
  } 
}
?>
<!-- Content Wrapper. Contains page content -->
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0">ADD SHOP FOR SALE SECTION</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
           <li class="<?php echo ($role==1 or $role==3)?'':' hide';?> btn btn-sm btn-primary" style="float: right; margin-right: 5px;">
            <a href="<?php _link(); ?>shops_for_sale"  aria-hidden="false" title="Back to Shops for sale" data-toggle="tooltip" data-placement="right" style="color: white;"><i class="fa fa-fast-backward"></i> &nbsp;Back </a>&nbsp;&nbsp; 
            </li>   
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
                ...
                <div class="card-body">
                  <div class="row">
                    ...
                    <div class="form-group col-md-2">
                      ...
                      <input type="text" name="store_room" class="form-control"  placeholder="Enter Longitude">
                    </div>
                    <div class="form-group col-md-2">
                    ...
                    <div class="form-group col-md-2">
                     ...
                     <div class="form-group col-md-2">
                      ...
                      <select name="shop_type" class="form-control select2" style="width: 100%;">
                        <option selected="selected">Choose shop type</option>
                    ...
                    <div class="form-group col-md-12">
                      ...
                      <textarea class="form-control" name="description" rows="4" placeholder="General Shop Description..."></textarea>     
                    </div>
                  </div>
                <!-- /.row -->
