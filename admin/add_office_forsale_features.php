...
<!-- Content Wrapper. Contains page content -->
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-12">
            <h1 class="m-0">OFFICES FOR SALE FEATURES SECTION
              <li class="btn btn-sm btn-primary"style="float: right;">
                    <a href="<?php _link(); ?>offices_for_sale" aria-hidden="false"  title="View offices for sale" data-toggle="tooltip" data-placement="right" style="color: white;"><i class="fa fa-fast-backward"></i>&nbsp;Go to offices for sale</a>
                    </li> </h1>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
          ...
          <div class="col-md-12">
            ...
            <div class="card card-primary">
              <div class="card-header">
                <?php
                    $get_office = dbQuery("SELECT * FROM tbl_office_for_sale WHERE office_code ='$office_code' LIMIT 1");
                      $count1 = dbNumRows($get_office);
                    if($count1 > 0) {
                      while($row1=dbFetchAssoc($get_office)){ 
                      $name = $row1['office_name']; ?>
                <h3 class="card-title">Add office Features for : <?php echo $name; ?> </h3>
              <?php } } ?>
              </div>
              <!-- /.card-header -->
              <!-- form start -->
               <form action="features/OSF_update.php" method="post">
<input type="hidden" id="office_code" name="office_code" value="<?php echo $office_code;?>" required>
                <div class="card-body">
                  <div class="row col-md-12 alert alert-success">
                    <?php
                    $get_features = dbQuery("SELECT * FROM tbl_office_forsale_features WHERE office_code ='$office_code' LIMIT 1");
     ...
     <div class='alert alert-danger'>
        <button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
               This office has no extra features, add features by selecting from the list below.
      </div>";
                 } ?>
              </div>
                <!-- /.card-body -->
