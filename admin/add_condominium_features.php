...
<!-- Content Wrapper. Contains page content -->
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-12">
            <h1 class="m-0">CONDOMINIUMS FOR SALE FEATURES SECTION
              <li class="btn btn-sm btn-primary"style="float: right;">
                    <a href="<?php _link(); ?>condominiums_for_sale" aria-hidden="false"  title="View Condominiums for sale" data-toggle="tooltip" data-placement="right" style="color: white;"><i class="fa fa-fast-backward"></i>&nbsp;Go to condominiums for sale</a>
                    </li> </h1>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
          ...
          <div class="col-md-12">
            ...
            <div class="card card-primary">
              <div class="card-header">
                <?php
                    $get_condo = dbQuery("SELECT * FROM tbl_condominiums WHERE condo_code ='$condo_code' LIMIT 1");
                      $count1 = dbNumRows($get_condo);
                    if($count1 > 0) {
                      while($row1=dbFetchAssoc($get_condo)){ 
                      $name = $row1['condo_name']; ?>
                <h3 class="card-title">Add Condominium Features for : <?php echo $name; ?> </h3>
              <?php } } ?>
              </div>
              <!-- /.card-header -->
              <!-- form start -->
               <form action="features/Condo_features_update.php" method="post">
<input type="hidden" id="condo_code" name="condo_code" value="<?php echo $condo_code;?>" required>
                <div class="card-body">
                  <div class="row col-md-12 alert alert-success">
                    <?php
                    $get_features = dbQuery("SELECT * FROM tbl_condominium_features WHERE condo_code ='$condo_code' LIMIT 1");
     ...
     <div class='alert alert-danger'>
        <button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
               This condo has no extra features, add features by selecting from the list below.
      </div>";
                 } ?>
              </div>
                <!-- /.card-body -->
