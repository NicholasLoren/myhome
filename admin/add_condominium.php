...
if (isset($_POST['submit'])) {
  ...
  //creating death code
  $result = @ dbQuery("SHOW TABLE STATUS LIKE  'tbl_condominiums'");
  $data = @dbFetchAssoc($result);
  $next = $data['Auto_increment'];
  $prefix = "MYHOME/C/";
  $condo_code = sprintf("%s%03s",$prefix,$next);
                    
  $existTime = dbNumRows(dbQuery("SELECT * FROM `tbl_condominiums` WHERE `condo_name`='$condo_name'"));
  if($existTime>0){
    warningMsg(" Condominium Already registered");
    }else{
        
  //insert  into database
  $condo_sale = dbQuery("INSERT INTO tbl_condominiums(`condo_code`, `condo_name`, `location`, `longitude`, `latitude`, `price`, `measurement`,  `description`, `condo_type`, `year_built`, `accomodation_type`, `bedrooms`, `bathrooms`, `dining_room`, `sitting_room`, `visitors_room`, `office_room`, `kitchen`, `cooling`, `heating`, `parking`, `outdoor`, `swimming_pool`, `total_floors`, `living_area`, `listed_by`) VALUES('$condo_code', '$condo_name', '$location', '$longitude', '$latitude', '$price', '$measurement', '$description', '$condo_type', '$year_built', '$accomodation_type', '$bedrooms', '$bathrooms', '$dining_room', '$sitting_room', '$visitors_room', '$office_room', '$kitchen', '$cooling', '$heating', '$parking', '$outdoor', '$swimming_pool', '$total_floors', '$living_area','$listed_by')"); 
   
      if($condo_sale){
       //insert condo code into table condo features
     dbQuery("INSERT INTO tbl_condominium_features(`condo_code`)VALUES('$condo_code')");

         $msg = "
     <div class='alert alert-success alert-dismissible'>
        <button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
                New Condominium Successfully Registered
      </div>";
      goToPage("condominiums_for_sale");
    }
  } 
}
?>
<!-- Content Wrapper. Contains page content -->
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0">ADD CONDOMINIUM FOR SALE SECTION</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
                ...
                <div class="card-body">
                  <div class="row">
                    <div class="form-group col-md-6">
                      <label for="exampleInputtext1">Condominium Name</label>
                      <input type="text" name="name" class="form-control"  placeholder="Enter Condominium Name">
                    </div>
                    <div class="form-group col-md-6">
                    ...
                    <div class="form-group col-md-2">
                      ...
                      <select name="condo_type" class="form-control select2" style="width: 100%;">
                        <option selected="selected">Choose Condo type</option>
