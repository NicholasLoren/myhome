...
if (isset($_POST['submit'])) {
  ...
  //creating death code
  $result = @ dbQuery("SHOW TABLE STATUS LIKE  'tbl_home_rentals'");
  $data = @dbFetchAssoc($result);
  $next = $data['Auto_increment'];
  $prefix = "MYHOME/HR/";
  $home_code = sprintf("%s%03s",$prefix,$next);
                    
  $existTime = dbNumRows(dbQuery("SELECT * FROM `tbl_home_rentals` WHERE `home_name`='$home_name'"));
  if($existTime>0){
    warningMsg("Rental Home Already registered");
    }else{
        
  //insert  into database
  $home_rental = dbQuery("INSERT INTO tbl_home_rentals(`home_code`, `home_name`, `location`, `longitude`, `latitude`, `price`, `period`, `measurement`,  `description`, `home_type`, `year_built`, `accomodation_type`, `bedrooms`, `bathrooms`, `dining_room`, `sitting_room`, `visitors_room`, `office_room`, `kitchen`, `cooling`, `heating`, `parking`, `outdoor`, `swimming_pool`, `total_floors`, `living_area`, `listed_by`) VALUES('$home_code', '$home_name', '$location', '$longitude', '$latitude', '$price', '$period', '$measurement', '$description', '$home_type', '$year_built', '$accomodation_type', '$bedrooms', '$bathrooms', '$dining_room', '$sitting_room', '$visitors_room', '$office_room', '$kitchen', '$cooling', '$heating', '$parking', '$outdoor', '$swimming_pool', '$total_floors', '$living_area','$listed_by')"); 

      if($home_rental){
        //insert home code into table home features
     dbQuery("INSERT INTO tbl_rental_home_features(`home_code`)VALUES('$home_code')");

         $msg = "
     <div class='alert alert-success alert-dismissible'>
        <button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
                New Rental Home Successfully Registered
      </div>";
      goToPage("listHome_rentals");
    }
  } 
}
?>
<!-- Content Wrapper. Contains page content -->
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0">RENTAL HOMES SECTION</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
                  ...
                  <div class="row">
                    ...
                    <div class="form-group col-md-2">
                      ...
                      <select name="home_type" class="form-control select2" style="width: 100%;">
                        <option selected="selected">Choose Home type</option>
                        <option value="Family Apartment">Family Home</option>
