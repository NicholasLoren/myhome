...
if (isset($_POST['submit'])) {
  ...
  //creating death code
  $result = @ dbQuery("SHOW TABLE STATUS LIKE  'tbl_office_for_sale'");
  $data = @dbFetchAssoc($result);
  $next = $data['Auto_increment'];
  $prefix = "MYHOME/OFS/";
  $office_code = sprintf("%s%03s",$prefix,$next);
                    
  $existTime = dbNumRows(dbQuery("SELECT * FROM `tbl_office_for_sale` WHERE `office_name`='$office_name'"));
  if($existTime>0){
    warningMsg("Office for sale Already registered");
    }else{
        
  //insert  into database
  $office_sale = dbQuery("INSERT INTO tbl_office_for_sale(`office_code`, `office_name`, `total_rooms`, `location`, `longitude`, `latitude`, `price`, `measurement`,  `description`, `office_type`, `accomodation_type`, `year_built`, `waiting_room`, `cooling`, `heating`, `parking`, `outdoor`, `listed_by`) VALUES('$office_code', '$office_name', '$total_rooms','$location', '$longitude', '$latitude', '$price', '$measurement', '$description', '$office_type',  '$accomodation_type', '$year_built', '$waiting_room','$cooling', '$heating', '$parking', '$outdoor','$listed_by')"); 

      if($office_sale){
        //insert office features
     dbQuery("INSERT INTO tbl_office_forsale_features(`office_code`)VALUES('$office_code')");
         $msg = "
     <div class='alert alert-success alert-dismissible'>
        <button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
                New Office for sale Successfully Registered
      </div>";
      goToPage("offices_for_sale");
    }
  } 
}
?>
<!-- Content Wrapper. Contains page content -->
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0">OFFICES FOR SALE SECTION</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
